...
{
    Uint8 *p =
        (Uint8 *)img->pixels + y * img->pitch + x * img->format->BytesPerPixel;
    Uint8 r, g, b;
...
void color_to_mask2(SDL_Surface *img, Uint8 *mask)
{
    ...
    if (mean > 240)
        ...
        thr = (Uint8)(mean * 0.45); // The I is missing but it is splited, last
                                    // thing to fix
    ...
    for (int m = 0; m < nby; m++)
    {
        ...
        for (int y = y0; y <= y1; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                char name[128];
                snprintf(name, sizeof(name), "%s_r%02d_c%02d.bmp", prefix, m,
                         nb_letters++);
...
SDL_Surface *rotateSurface(SDL_Surface *src, double angle)
{
    ...
    SDL_Surface *dst = SDL_CreateRGBSurface(
        0, newW, newH, src->format->BitsPerPixel, src->format->Rmask,
